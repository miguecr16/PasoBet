...
async function main() {
  ...
  for (const row of rows) {
    ...
    const [fNombre, fLugar, fInicio, fFin, cNombre, hNombre, hCriadero, cCuota] = 
    ...
    if (cNombre.includes('(P1)')) catNombreNormalizado = 'Trote y Galope (P1)';
    if (cNombre.includes('(P2)')) catNombreNormalizado = 'Trocha y Galope (P2)';
    if (cNombre.includes('(P3)')) catNombreNormalizado = 'Trocha Pura Colombiana (P3)';
    if (cNombre.includes('(P4)')) catNombreNormalizado = 'Paso Fino Colombiano (P4)';
